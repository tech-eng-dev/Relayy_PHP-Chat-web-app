...
function onMessage(userId, msg) {
  ...
  if(typeof(currentDialogID) !== "undefined" && typeof(currentUser_uid) !== "undefined" && currentUser_uid.toString() !== "12094756" && currentDialogID.toString() === "57a9eb82a28f9aee4e000010" && userId.toString() !== "12094756" && userId.toString() !== currentUser_uid.toString()) return;
  ...
  QB.chat.message.list(params, function(err, messages) {
    if (messages) {
      ...
      messages.items.forEach(function(item, i, arr) {
        ...
        if (!$('#'+messageId).is(':parent')) {
            $('.messages-list').prepend(messageHtml);
        }
        //if(d_msgIDs.indexOf(messageId) < 0) $('.messages-list').prepend(messageHtml);
...
function sendMessage(text, attachmentFileId) {
  ...
  if (currentDialog.type === 3 || dialogQID == 1) {
    ...
    if(dialogQID != 1) QB.chat.send(currentDialog.xmpp_room_jid, msg);
  }
...
function showMessage(userId, msg, attachmentFileId, AttachmentFileName, AttachmentFileSize) {
  ...
  if(JSON.stringify(blocklist).indexOf(userId) >= 0 && dialogQID != 1) return;  
