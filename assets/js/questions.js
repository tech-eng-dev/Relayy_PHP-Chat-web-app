...
function onAdvisorSearch(){
  ...
  $(".advisor_contacts").each(function(){
    ...
    if($(this).find('span:first').text().toLowerCase().indexOf($("#advisor_name").val().toLowerCase()) < 0){
      $(this).css('display', 'none');
      return;
    }

    var keyword = $("#advisor_skill").val().replace(/ /g, '');
    var keywords = keyword.split(",");

    for (var i = 0; i<keywords.length; i++) {
      if($(this).attr('data-key').toLowerCase().indexOf(keywords[i].toLowerCase()) < 0){
        $(this).css('display', 'none');
        return;
      }
    };
...
function passFeed(obj, q_id, c_id, b_accept, body_class){
  if(q_id == 1){
    alert('This button will remove a question and refine the quality of future questions that are sent to you.');
    return;
  }
  if(!b_accept) $(obj).find("#pass-spinner").show();
  else{
    $(".common_question_alert").html("<center class='gray-text'>Loading next question...</center>");
    if(val_feed_alert == 1) $(".common_question_alert").html("<center class='gray-text'>Please wait...</center>");
  }
...
// store 3 params to show next question after clicked "Next Question" or "Don's show this message again" button.
var fobj, fq_id, fc_id;

function acceptFeed(obj, asker_email, asker_uid, c_id, q_id){
  if(q_id == 1){
    location.href = site_url + "chat/channel/57e5ea9aa0eb47d9de00005b";
    return;
  }
	var params = {
		type: 2,
  ...
  $(obj).find("#join-spinner").show();
	    ...
	    $.ajax({
         ...
         success: function(data) {
            passFeed(obj, q_id, c_id, true, "question-feed");
         },
         type: 'POST'
      });	                         
