...
function LoginUser(uid, email, fname, lname, picture, bio, role, location, public_url, company){
    $.ajax({
       ...
       success: function(data) {
          if(data === "not_active"){
              $("#validating_text").text("You are not active user. You must sign up now.");
              return;
          }
          else if(data === "invited_user"){
              $("#invite_notif_msg").show();
              $("#invite_notif_msg_text").text();
          }
          else{
              $("#validating_text").text("Verified successfully! Logging in now...");
              $("#li_id").val(uid);
              $("#li_fname").val(fname);
              $("#li_lname").val(lname);
              $("#li_email").val(email);
  ...
  QB.users.listUsers(filters, function(err, result){
    if (result && result.items.length> 0) {//================= Old User in QuiclBlox
      ...
      $.ajax({
       ...
       success: function(data) {
          if(role == 2) window.location.href = site_url + 'home/onboarding_advisor';
          if(role == 3) window.location.href = site_url + 'home/onboarding_entrepreneur';
          else if(role == 4) window.location.href = site_url + 'home/onboarding_moderator';
       },
       type: 'POST'
      });
    } else if (result && result.items.length == 0) {
