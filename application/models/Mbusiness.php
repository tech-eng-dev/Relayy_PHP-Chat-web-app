...
class Mbusiness extends CI_Model {
    ...
    public function getTopEntreps(){
      $query = $this->db->select('tbl_business_profile.id as id, tbl_business_profile.challenge as challenge, tbl_user.fname as fname, tbl_user.lname as lname, tbl_user.photo as photo') // (or whichever fields you're interested in)
                        ->from("tbl_business_profile")
                        ->join("tbl_user", "tbl_user.id = tbl_business_profile.id")
                        ->where('tbl_business_profile.challenge !=', "")
                        ->get();
        return $query->result_array();
    }
