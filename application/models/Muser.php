...
class Muser extends CI_Model {
    ...
    public function add( $data_arr )
    {
        ...
        if($newUser->row()->{TBL_USER_TYPE} % 10 == 2){
            $this->db->insert(TBL_NAME_OPTION, array(
              TBL_OPTION_UID => $id,
              TBL_OPTION_UNREAD => 1,
              TBL_OPTION_INVITE => 2,
              TBL_OPTION_COMMENT => 1,
              TBL_OPTION_REVIEW => 1,
              TBL_OPTION_ROUTE => 2
              ));
        }
        else if($newUser->row()->{TBL_USER_TYPE} % 10 == 3){
            $this->db->insert(TBL_NAME_OPTION, array(
              TBL_OPTION_UID => $id,
              TBL_OPTION_UNREAD => 1,
              TBL_OPTION_INVITE => 2,
              TBL_OPTION_APPROVE => 1,
              TBL_OPTION_COMMENT => 1,
              TBL_OPTION_REVIEW => 1
              ));
        }
    ...
    public function updateLink($link, $id){
        $this->db->update(TBL_NAME_USER, array(TBL_USER_PUBLIC => $link), array(TBL_USER_ID => $id));
        return TRUE;
    }
    ...
    public function getEmail($where_email)
    {
        $query = $this->db->select('*')
                          ...
                          ->or_where(TBL_USER_LINKEDIN_EMAIL, $where_email)
                          ->limit(1)
                          ->get(TBL_NAME_USER);
    ...
    //     $query = $this->db->select('*')
    //                       ->where(TBL_USER_STATUS, )
    //                       ->where(TBL_USER_EMAIL, $email)
    //                       ->or_where(TBL_USER_LINKEDIN_EMAIL, $email)
    //                       ->limit(1)
    //                       ->get(TBL_NAME_USER);
    ...
    public function getUserwithNotificatinEmail($email){
      $query = $this->db->select('*')
                          ->where(TBL_USER_EMAIL, $email)
                          ->limit(1)
                          ->get(TBL_NAME_USER);
    ...
    public function delete($id)
    {
        ...
        //format in tbl_invite
        $this->db->where(TBL_INVITE_ID, $id);
        $this->db->delete(TBL_NAME_INVITE);

        //format in tbl_business_profile
        $this->db->where(TBL_BUSINESS_ID, $id);
        $this->db->delete(TBL_NAME_BUSINESS);

        //check group name is necessar or not
        $query = $this->db->select('*')
                          ->where(TBL_USER_ID, $id)
                          ->limit(1)
                          ->get(TBL_NAME_USER);
        $user = $query->row();
        $groupcode = $user->{TBL_USER_GROUP};

        if($user->{TBL_USER_TYPE} == 4 && $user->{TBL_USER_STATUS} == USER_STATUS_LIVE){//If deleting only an active moderator, group can be disappeared
            $query = $this->db->select('*')
                              ->where(TBL_USER_GROUP, $groupcode)
                              ->where(TBL_USER_TYPE, USER_TYPE_MODERATOR)
                              ->get(TBL_NAME_USER);


            if ($query->num_rows() === 1)//need to delete group
            {
                $this->db->where(TBL_GROUP_CODE, $groupcode);
                $this->db->delete(TBL_NAME_GROUP);

                $this->db->where(TBL_USER_INVITE_GROUP, $groupcode);
                $this->db->delete(TBL_NAME_USER_INVITE);

                $this->db->update(TBL_NAME_USER, array(TBL_USER_GROUP => "", TBL_USER_CODE => ""), array(TBL_USER_GROUP => $groupcode));
            }
        }
    ...
    public function DeletefromGroup($id){
      $query = $this->db->select('*')
                          ...
                          ->get(TBL_NAME_USER);
      $user = $query->row();
      if($user->{TBL_USER_STATUS} != 1) $this->delete($id);
      else if($user->{TBL_USER_TYPE} == 2 || $user->{TBL_USER_TYPE} == 3) $this->db->update(TBL_NAME_USER, array(TBL_USER_GROUP => "", TBL_USER_CODE => ""), array(TBL_USER_ID => $id));
      else if($user->{TBL_USER_TYPE} == 4){
        $this->db->update(TBL_NAME_USER, array(TBL_USER_TYPE => USER_TYPE_ADVISOR, TBL_USER_GROUP => "", TBL_USER_CODE => ""), array(TBL_USER_ID => $id));
