...
class Mgroup extends CI_Model {
    ...
    public function delete($id, $group, $type){
        ...
        $this->db->where(TBL_USER_INVITE_GROUP, $group);
    ...
    public function inviteNonGroupUser($whomid, $group, $role, $whoid){
        $query = $this->db->select('*')
                        ->where(TBL_USER_INVITE_WHOM, $whomid)
                        ->limit(1)
                        ->get(TBL_NAME_USER_INVITE);
        if ($query->num_rows() === 1){
            return FALSE;
        }
        $this->db->insert(TBL_NAME_USER_INVITE, array(
                                                    TBL_USER_INVITE_WHOM => $whomid,
                                                    TBL_USER_INVITE_GROUP => $group,
                                                    TBL_USER_INVITE_ROLE => $role,
                                                    TBL_USER_INVITE_WHO => $whoid)
                        );
        $id = $this->db->insert_id();
        return $id;
    }

    public function getInvitedNonGroupUsers(){
        $query = $this->db->select('tbl_user_invite.whom_id, tbl_group.name')
                        ->from(TBL_NAME_USER_INVITE)
                        ->join(TBL_NAME_GROUP, "tbl_group.code = tbl_user_invite.invite_group", 'left')
                        ->get();
        return $query->result_array();    
    }

    public function checkGroupInvite($id){
        $query = $this->db->select('*')
                        ->where(TBL_USER_INVITE_WHOM, $id)
                        ->limit(1)
                        ->get(TBL_NAME_USER_INVITE);
    ...
    public function DeclineGroupInvite($id){
        $this->db->where(TBL_USER_INVITE_WHOM, $id);
        $this->db->delete(TBL_NAME_USER_INVITE);
    }
