...
class MY_Email extends CI_Email {
    ...
    {
        if(strpos(site_url(), "staging") !== false) $toEmail = "staging@relayy.io";
                                            ...
                                            '</tr>';
                                                    ...
                                                    foreach(json_decode($summary->unread) as $emt){
                                                        ...
                                                        $msg .= '<tr>'.
                                                                '<td style="background:white;padding:0px 20px;">'.
                                                                    '<p style="margin:0px 0px 10px 0px;"><a href="'.base_url().'chat/channel/'.$data->did.'">'.$data->num.' unread messages</a> in '.$data->name.' chat with '.$data->creator.'</p>'.
                                                                '</td>'.
                                                                '</tr>';
                                                    }
                                                }
                                                ...
                                                if(strlen($summary->review) > 2){
                                                    $msg .= '<tr>'.
                                                            ...
                                                            '</td>'.
                                                            '</tr>';
                                                    foreach(json_decode($summary->review) as $emt){
                                                        $msg .='<tr>'.
                                                                '<td style="background:white;padding:0px 20px;">'.
                                                                    '<p style="margin:0px 0px 10px 0px;">'.$emt->name.' just <a href="'.base_url().'profile/user/'.$emt->id.'">posted a review to your profile</a></p>'.
                                                                '</td>'.
                                                                '</tr>';
                                                    }
                                                }
    ...
    public function sendEmailNotification($sender, $MessageText, $inviteLink, $toEmail, $toName)
    {
        $this->sendEmail(
            ...
            "We would like to help. Please email use with any converns at support@relayy.io.<br>Change your email preferences <a href='".site_url()."setting'>[here]</a>"
        );
    }
    ...
    public function sendRefer($fromName, $fromEmail, $Msg, $toName, $toEmail, $qid){
        $this->sendEmail(
            ...
            "We would like to help. Please email us with any concerns at support@relayy.io Change your email preferences <a href='".site_url()."setting'>[here]</a>"
        );  
    }
    ...
    public function inviteChat($inviterName, $chatName, $chatDesc, $EmailArray, $inviteLink, $toEmail, $toName = "Hi", $inviterEmail)
    {
        ...
        $this->sendEmail(
            ...
            "You were invited to this chat by ".$inviterEmail.".<br>We would like to help. Please email us with any concerns at support@relayy.io Change your email preferences <a href='".site_url()."setting'>[here]</a>"
        );   
    } 

    public function sendInviteDeclineEmail($toEmail, $name, $email, $group){//$toemail: moderator's email who invited,      $email: the user's email who was invited
        $this->sendEmail(
            "notifications@relayy.io", 
            "Relayy", 
            $toEmail, 
            "Invitation Declined",
            "",
            $name.", ".$email.", declined the invitation to join your group: [".$group."].<br><br>Please contact this user ".$email." for any follow up questions.",
            ""
        );   
    }

    public function sendGroupInvitation($inviterName, $toName, $toEmail, $group, $inviterEmail)
    {
        $this->sendEmail(
            "notifications@relayy.io", 
            "Relayy", 
            $toEmail, 
            $inviterName." invited you to a Relayy Group",
            $toName.", you've been invited to a group on Relayy.",
            "Group:".$group."<br>Invited by:".$inviterName."<br><br><a href='".site_url()."'>Log in</a> to accept or decline this invitation.<br><br>
            How do Groups work?<br>Relayy Groups allow users to ask and answer questions inside of a private group-instead of using the entire Relayy network. Group members are still able to participate in the open Relayy network if they so desire.",
            "Please contact this user ".$inviterEmail." for any follow up questions."
        );   
    }
