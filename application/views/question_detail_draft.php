...
<div class="container-widget question-container canvas" style="height:100vh;">
...
<div class="desktop-visible-item">
      ...
      <div class="row white_back content-title-bar">
        ...
        <div class="padding_xs pull-right">
...
<div class="row border1234 white_back padding_xs col-text">
  <div class="col-sm-9 col-xs-12 canvas">
    ...
    <div class="explore-section">
      <h5 class="col-xs-12 BT question-lamp-text" style="margin:7px 0px;">
        Not sure where to start? Need some question suggestions?
      </h5>
    </div>
    <div class="explore-section gray-80">
...
<?php foreach($array_question as $question){?>
            ...
            <div class="question-style box-section" data-time="<?= $question['time'] ?>" data-cardid="<?= $question['q_id'] ?>" id="card_<?= $question['q_id'] ?>">
                              ...
                              <div class="dropdown col-sm-3 col-xs-6 pull-right">
                                <center>
                                  ...
                                  <div class="dropdown-content">
                                      ...
                                      <button class="trans full-width" style="margin:0px;" onclick="RouteQuestion(<?= $question['q_id']?>)"><a>Route</a></button>
                                                                      
                                      <?php if(($u_type == 1 || $u_type == 4) && sizeof(json_decode($question['a_ids'])) > 0) {?>
                                        <button class="trans full-width" style="margin:0px;" onclick="CreateTeamUp(<?= $question['q_id']?>)"><a>Create TeamUps</a></button>
                                      <?php }?>  
                                    
                                      <?php if($u_type == 4){?>
                                        <button class="trans full-width togglePostState" style="margin:0px;" data-id="<?= $question['q_id'] ?>" data-state = "<?= $question['post'] ?>"><a>BroadCast</a></button>
                                      <?php }?> 

                                      <button class="trans full-width" style="margin:0px;" onclick="referToColleague('<?= $question['q_id']?>', '<?= $question['title']?>')"><a>Refer Question</a></button>

                                      <?php if($question['state'] != 4){?>
                                        <button class="trans full-width" style="margin:0px;" onclick="deleteQuestion('<?= $question['q_id']?>')"><a>Delete</a></button>
                                      <?php } ?>
...
<!-- --------------------------------------------------------------  Waiting Button -------------------------------------- -->
                ...
                <div class="row question-about padding_sm">
                  <div class="row">
                    <?php 
                      $Title = $question['title'];//change sample question's color
                      if($question['q_id'] == 1){
                        $Title = str_replace("Sample Question:", "<span class='pull-left' style='color:#F34E4E'>Sample Question:</span>", $Title);
                      }
                    ?>
                    <h4 class="desktop-visible-item BT gray-80" style="margin:0px;"><?= $Title?></h4>
                    <h5 class="mobile-visible-item BT gray-80" style="margin:0px;"><?= $Title?></h5>
                  </div>
