...
<script>
  ...
  var dialogQID = "<?php echo $d_qid ?>";
  var saved_mids = '<?php echo isset($saved_mids)?$saved_mids:"" ?>';
  var liked_mids = '<?php echo isset($liked_mids)?$liked_mids:"" ?>';
  var b_send;
  var keys = {
    enter: false,
    ctrl: false
  };
  var d_msgIDs = '<?php echo isset($d_messages)?$d_messages:"" ?>';
  currentDialogID = "<?php echo $d_current ?>";

  removeBadge(dialogID);
  triggerDialog(dialogID, 1);
  setupAllListeners();
...
<?php if(isset($deleted) && $deleted == 1 && $d_qid != 1){ ?>
<center style="margin-top:200px;">Sorry, the chatroom is invalid, perhaps it was deleted.</center>
<?php } else if(isset($d_users)) { ?>

<!-- <div class="section_wrapper"> -->
<div class="white_back desktop-visible-item content-title-bar">
  
  
  <?php if($d_type == 0){ ?>
    <h4 class="pull-left gray-text">How can I help you?</h4>
  <?php } else { ?>
    <h4 class="pull-left gray-text"><?= $d_name?></h4>
  <?php } ?>

  <?php if ($d_owner == "Me" && $d_type != 0) {?>
    <button class="pull-right rb delete-chat" onclick="deleteAction('<?= $d_id?>')" <?php echo $d_qid == 1?"disabled='disabled'":"" ?>><span class="btn-icon glyphicon glyphicon-trash"></span>DELETE CHAT</button>
  <?php }?>
          ...
          <ul class="sidebar-panel nav scrollbar full-height">
                <li id="mobile-right-back" class="padding_xs chat-detail-title">
                  <div class="row">
                    ...
                    <button class="pull-right rb" onclick="deleteAction('<?= $d_id?>')" <?php echo $d_qid == 1?"disabled='disabled'":"" ?>>
                ...
                <li class="border3">
                  ...
                  <?php if($d_qid != 0) {?>
                    <center><a href="<?= site_url().'questions/question/'.$d_qid ?>" target="_blank">View Question Details</a></center>
                  <?php } ?>
                ...
                <li class="border3">
                  ...
                  <?php if($d_name !== "Private" && ($u_type != 3 || $d_owner == "Me")) {?>
                    <div class="row padding_xs"><center><button onclick="addMember('<?= $d_id?>', <?= str_replace('"', '',json_encode($d_occupants)) ?>)" class="ob" <?php echo $d_qid == 1?"disabled='disabled'":"" ?>><span class="glyphicon glyphicon-plus"></span>Add Members</button></center></div>
                  <?php } ?>
                </li>  
                <?php } ?>        
                ...
                ?>
                    <li>
                      ...
                      <ul class="dropdown-menu user-pop" style="width:200px;"> 
                                    <!-- if sample chatroom, make all actions disable -->
                                    <?php if($d_qid != 1){ ?>
                                    ...
                                    <?php } else { ?>
                                        <?php if($user['id'] == $u_id) { ?> <!-- ========== case you (don't pass here) ===== -->
                                          ...
                                          <li style="margin-left:10px;"><a>Profile</a></li>

                                        <?php } else if($user['status'] == USER_STATUS_DELETE) { ?> <!-- ========== case deleted user ===== -->

                                          <li style="background:lightblue;overflow:visible;height:50px;padding:5px;">
                                          <img class="round" src="<?= strlen($user['photo'])>0?$user['photo']:asset_base_url().'/images/emp-sm.jpg'?>" style="width:40px;heightL40px;float:left"> Deleted User</li>
                                          <li style="margin-left:10px;"><a>Remove</a></li>
                                        ...
                                        <?php } else if($user['email'] === 'jake@relayy.io'){ ?><!-- ========== case admin1 ===== -->
                                          ...
                                          <li style="margin-left:10px;"><a>Start 1:1 Chat</a></li>
                                          <li style="margin-left:10px;"><a>Profile</a></li>
                                        ...
                                        <?php } else { ?><!-- ========== case member ===== -->
                                          ...
                                          <li style="margin-left:10px;"><a>Block</a></li>
                                          <li style="margin-left:10px;"><a>Block and Report</a></li>     
                ...
                <li>
                  <div class="row padding_xs">
                    <center><button onclick="leaveAction('<?= $d_id?>')" class="rb" <?php echo $d_qid == 1?"disabled='disabled'":"" ?>>Leave this chat</button></center>
                  </div>
                </li>
                <?php } ?>
