...
<script type="text/javascript">
    ...
    function onLinkedInClk(b_login) {
    ...
    function onInviteCode(){
      ...
      $("#verify_code").show();
      $.ajax({            
           url: site_url + 'users/check_code_and_email',
           data: {
           ...
           success: function(data) {
              ...
              if(data.indexOf("exist") > -1){
                ...
                alertstate("You can't signup, because you are pending user in Relayy.");
                return;
              }
                ...
                alertstate('That invite code is invalid, or has been used the maximum amount of times allowed. Request an invite code on previous page.');
                  ...
                  registerFacebook(id, l_email, l_fname, l_lname, l_pictureUrl,
                  l_headline, 3, l_location, l_publicUrl, l_company, "");

              }
              else if(data.indexOf("Moderator") > -1){
                  registerFacebook(id, l_email, l_fname, l_lname, l_pictureUrl,
                  l_headline, 4, l_location, l_publicUrl, l_company, data.split("_")[1]);
              }
              else{//moderator's group  code
                registerFacebook(id, l_email, l_fname, l_lname, l_pictureUrl,
                  l_headline, 3, l_location, l_publicUrl, l_company, data);
              }
           },
           type: 'POST'
        });
    ...
    function onVerification() {
        ...
        var email = $("#auth_email").val().replace(/ /g, "");
        if(email == ""){
        ...
        $.ajax({            
           ...
           success: function(data) {
              ...
              else if(data == "invited_user"){
                alertstate("You can't login, because you are invited user. Please check email and signup with invite url in it.");
              }
              else if(data == "pending_user"){
    ...
    function VerifyEmail(){
        ...
        var token = document.getElementById("auth_token").value.replace(/ /g, "");  
        if(token == ""){
        ...
        if(token == VerifyCode){
          $.ajax({
             ...
             success: function(data) {
                ...
                else if(data.indexOf('home') > -1) location.href = site_url + data;
    ...
    function accept_invite_msg(){

    }

    function decline_invite_msg(){
...
<div id="main" class="main blue_main">
  ...
  <div class="padding_md" id="invite_notif_msg" style="display:none;">
    <center style="color:white;">
        <h4 id="invite_notif_msg_text"></h4>
        <button class="col-xs-6 btn" onclick="accept_invite_msg()">Accept</button>
        <button class="col-xs-6 rb" onclick="decline_invite_msg()">Decline</button>
    </center>
  </div>
...
<script>
  ...
  $(document).ready(function() {   
      ...
      log_state = '<?php echo isset($log_state)?$log_state:"" ?>';
      if(log_state === '1'){
        $("#validating_text").text("Verifying your data in Relayy...");
        LoginUser(id, l_email, l_fname, l_lname, l_pictureUrl,
                    l_headline, 3, l_location, l_publicUrl, l_company);
      }else{
