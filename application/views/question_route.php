...
<div class="container-widget"  style="height:100vh;margin-top:60px;padding:0px 10px;">    
    <div class="row" style="padding:15px;margin-top:35px;">
      <div class="row padding_xs">
        <div class="col-sm-6 col-xs-12 padding_xs">
          <p>Search by first and last name</p>
        </div>
      ...
      <div class="row padding_xs">
        <div class="col-sm-6 col-xs-12 padding_xs">
          <p id = "advisor_tag">Advisor Keywords</p>
          <p class="gray-text">Search advisor profiles using keywords. Separate words with a comma(e.g. marketing, advertisting).</p>
        </div>
        <div class="col-sm-6 col-xs-12">
    ...
    <div class="row round_table" style="height:50vh;">
        ...
        <div id="channel_edit" class="col-md-12 scrollbar" style="height:75%;overflow-x:hidden;overflow-y:auto;">
          <ul id="route_contacts">
              <?php $num = 0;
                    ...
                    foreach($advisors as $advisor){
                      ...
                      $advisor_key = str_replace('"', '', json_encode($advisor));
                  ?>
                  <li onclick="" class = "li-item advisor_contacts row" data-key="<?= $advisor_key ?>">
                    ...
                    <?php } else {?><div class="col-sm-1 col-xs-3"><a href="<?= site_url().'profile/user/'.$advisor['id'] ?>"><img class="avatar avatar_small" src="<?= asset_base_url()."/images/emp-sm.jpg"?>"></a></div>
                    <?php } ?>                        
                    <div class="col-sm-10 col-xs-8 container-widget">
                      <div class="row"><a href="<?= site_url().'profile/user/'.$advisor['id'] ?>" target="_blank" class="contacts_name" id="<?= $advisor['id']?>"><?= $advisor['fname'] ?> <?= $advisor['lname'] ?></a></div>
                      <div class="row"><span class="contacts_email" id="<?= $advisor['id']?>"><?= $advisor['fname']." ".$advisor['lname'] ?></span></div>
                    </div>
