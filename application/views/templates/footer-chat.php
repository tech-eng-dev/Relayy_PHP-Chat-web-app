...
if ($body_class == "users-page") {?>
    ...
    <script>
        function delAction(obj, email) {
            var delObj = $(obj);
            var alert;
            if(currentUser_type == 1) alert = 'Are you sure you want to delete this user: '+email+' ?';
            else alert = 'Are you sure you want to remove this user from your group: '+email+' ?';
        ...
        function sendInvite(userType) {
            var sendEmail = $("#invite_txt").val().replace(/ /g, "");
            if (sendEmail == '') {
            ...
            BootstrapDialog.show({
                ...
                buttons: [{
                    ...
                    action: function(dialogRef){
                        ...
                        $("#invite_txt").val('');
                        $.ajax({
                                   url: site_url + "users/invite/"+ userType + "/" + encodeURIComponent(sendEmail) + "/<?php echo isset($page)?$page:'0';?>",
                                   data: {
                                   },
                                   success: function(data) {
                                        if(data.indexOf("live_user") > -1){
                                            if(currentUser_type == 1) alertstate("The email belongs to an active Relayy member and thus can't be invited.");
                                            else alertstate('The email belongs to an active Relayy user: ' + data.split("/")[1] + '. Go to the "Search Relayy users" tab on the User Management page to invite this user to your group.');
                                            dialogRef.close();
                                            return;
                                        }
                                        else if(data.indexOf("group_user") > -1){
                                            if(currentUser_type == 1) alertstate("The email belongs to an active Relayy member and thus can't be invited.");
                                            else alertstate('The email belongs to an active Relayy user that is already a member of your group: ' + data.split("/")[1]);
                                            dialogRef.close();
                                            return;
                                        }
                                        location.reload();
