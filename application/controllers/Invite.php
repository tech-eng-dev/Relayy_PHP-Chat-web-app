...
class Invite extends ChatController//change CI_Controller to ChatController on 6th July
{
	...
	{    
        ...
        if($object->{TBL_USER_STATUS} == USER_STATUS_INVITE){
            $CI =& get_instance();
            $arr = array(
                'invite_pass' => 'invite',
                'invite_id' => $uid, 
    ...
    public function check_email(){
        ...
        else if($res->{TBL_USER_ID} != $id) echo "active_user/".$res->{TBL_USER_FNAME}." ".$res->{TBL_USER_LNAME}."/".$res->{TBL_USER_EMAIL};
        else if($res->{TBL_USER_STATUS} == USER_STATUS_INVITE) echo "valid";
        else "invalid";
    }
    ...
    public function loadInviteGroupView(){
        $data = $this->getChatData();
        $res = $this->mgroup->checkGroupInvite($this->cid);
        if(!$res) echo "ignore";
        else{
            $data['invited'] = "yes";
            $group = $this->mgroup->get($res->{TBL_USER_INVITE_GROUP});
            $who = $this->muser->get($res->{TBL_USER_INVITE_WHO});
            $data['group_image'] = $group[TBL_GROUP_IMAGE];
            $data['group_name'] = $group[TBL_GROUP_NAME];
            $data['inviter_photo'] = $who->{TBL_USER_PHOTO};
            $data['inviter_name'] = $who->{TBL_USER_FNAME}." ".$who->{TBL_USER_FNAME};
            $data['inviter_bio'] = $who->{TBL_USER_BIO};
            $this->load->view('invite_group', $data);
        } 
    }
