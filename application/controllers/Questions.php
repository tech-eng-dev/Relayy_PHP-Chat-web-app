...
class Questions extends ChatController
{
	...
	public function index()
	{
    	...
    	if($this->ctype == 1){
    		...
    		$user_data = $this->mbusiness->getArray($this->cid);
    		if($user_data[TBL_BUSINESS_SKILL] === "" || $user_data[TBL_BUSINESS_SKILL] === "[]") $onboarding_advisor = 1;
    		else if($user_data[TBL_BUSINESS_INTERESTING] === "" || $user_data[TBL_BUSINESS_INTERESTING] === "[]") $onboarding_advisor = 2;
    		$opt = $this->moption->get($this->cid);
    		$chat_data['feed_alert'] = $opt[0][TBL_OPTION_FEEDALERT];
	...
	public function AcceptQuestion(){
		...
		$data_arr = array(
			...
			TBL_FEED_GROUP => $asker->{TBL_USER_GROUP}
		);
		$this->mfeed->add($data_arr);
    ...
    public function NeverFeedAlert(){
    	$id = $this->input->post('u_id');
    	$this->moption->update(array(TBL_OPTION_FEEDALERT => 0), $id);
    	echo "success";
    }
