...
class Chat extends ChatController
{
	...
	public function channel($current_id)
	{
        ...
        if(!$find){
            ...
            $chat_data['d_current'] = $current_id;

            $chat_data['d_qid'] = $current_id;
    ...
    public function saveMessage(){
        ...
        $data_arr = array(
            ...
            TBL_FEED_GROUP => $whomUser->{TBL_USER_GROUP}
        );
    ...
    public function getPhoto(){
        ...
        if(strlen($user->{TBL_USER_PHOTO}) == 0) echo asset_base_url().'/images/emp.jpg'."q.q".$user->{TBL_USER_ID};
            ...
            if ($dialog['did'] == $did) {
                ...
                foreach ($chat_data['d_occupants'] as $d_user) {
                    ...
                    if($opt[0][TBL_OPTION_UNREAD] == 1){//summary
                        $d_owner = $this->muser->get($chat_data['d_occupants'][0]);
                        $name_owner = $d_owner->{TBL_USER_FNAME}." ".$d_owner->{TBL_USER_LNAME};
                        $this->updateUnread($dialog['name'], $dialog['did'], $tempUser[TBL_USER_ID], $name_owner);
                    }
                    else if($opt[0][TBL_OPTION_UNREAD] == 2 && $currentTime - $tempUser[TBL_USER_TIME] > 100){//instance message
                        $this->email->sendEmailNotification($this->cfname, $msg, site_url()."chat/channel/".$dialog[TBL_CHAT_DID], $tempUser[TBL_USER_EMAIL], $tempUser[TBL_USER_FNAME]);
    ...
    public function updateUnread($chatName, $did, $id, $name_owner){
                ...
                if ($occupant[1] == "") {
                    ...
                    if ($oldUser) {
                        ...
                        $data_arr = array(
                            ...
                            TBL_USER_LINKEDIN_EMAIL => strtolower($occupant[0])                            
                        );
