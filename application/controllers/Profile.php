...
class Profile extends ChatController
{
	...
	public function index()
	{
		...
		if($this->ctype == 4) $chat_data['n_moderator'] = $this->mcode->checkModeratorNum($this->cgroup);
	...
	public function user($c_id)
	{
		...
		$chat_data['current_uid'] = $user_data[TBL_USER_UID];
		...
		$chat_data['n_moderator'] = $this->mcode->checkModeratorNum($user_data[TBL_USER_GROUP]);
		$chat_data['group'] = $user_data[TBL_USER_GROUP];
		$chat_data['group_name'] = $mygroup[TBL_GROUP_NAME];
		$chat_data['group_image_name'] = $mygroup[TBL_GROUP_IMAGE];
	...
	public function SaveUserPosition(){
		...
		$this->mbusiness->updatePosition($position, $id);
		echo "success";
	}
	...
	public function SaveUserEducation(){
		$education = $this->input->post('education');
		$id = $this->input->post('id');
		$this->mbusiness->updateEducation($education, $id);
		echo "success";
	}
	...
	public function updateUserVentureInfo(){
		$category = $this->input->post('category');
		$data = $this->input->post('data');
		$id = $this->input->post('id');
		$data_arr = array($category=>$data, TBL_BUSINESS_ID=>$id);
		echo $this->mbusiness->updateInfo($data_arr);
	...
	public function updateLink(){
		...
		echo $this->muser->updateLink($link, $this->cid);
	}

	public function updateUserLink(){
		$link = $this->input->post('link');
		$id = $this->input->post('id');
		echo $this->muser->updateLink($link, $id);
	}
	...
	public function LeaveGroupForModerator(){//uncalled because sub-moderator was removed
		$id = $this->input->post('id');
		$type = $this->input->post('type');
		$n_moderator = $this->mcode->checkModeratorNum($this->cgroup);
		if($n_moderator > 1){
	...
	public function SaveCategory(){
		$category = $this->input->post('category');
		$id = $this->input->post('id');
		$this->muser->updateUser($id, array(TBL_USER_CATEGORY => $category));
		echo "success";
	}
	...
	public function addUserLink(){
		...
		$data_arr = array(TBL_LINK_ID=>$id, TBL_LINK_TITLE=>$title, TBL_LINK_IMAGE=>$fname, TBL_LINK_LINK=>$text);
		$this->mbusiness->addLink($data_arr);
		echo $fname;
	}
	...
	public function deleteUserLink(){
		$link = $this->input->post('link');
		$id = $this->input->post('id');
		echo $this->mbusiness->removeLink($id, $link);
	}	
	...
	public function SaveUserSkill(){
		...
		$this->mbusiness->saveBusinessData(array(TBL_BUSINESS_SKILL=>$skill, TBL_BUSINESS_ID=>$id, TBL_BUSINESS_UID=>$uid));
	}
	...
	public function SaveUserInteresting(){
		$interesting = $this->input->post('interesting');
		$id = $this->input->post('id');
		$uid = $this->input->post('uid');
		$this->mbusiness->saveBusinessData(array(TBL_BUSINESS_INTERESTING=>$interesting, TBL_BUSINESS_ID=>$id, TBL_BUSINESS_UID=>$uid));
	}
	...
	public function SaveUserCompanyInfo(){
		$category = $this->input->post('category');
		$value = $this->input->post('value');
		$id = $this->input->post('id');
		$this->muser->updateUser($id, array($category => $value));
		echo "success";
	}
	...
	public function edit($id)
	{
		...
		$user = $this->muser->getUserArray($id);
	...
	public function save($id)
	{
        ...
        $object = $this->muser->edit($id, array(
        		...
        		TBL_USER_PUBLIC => $linkedIn,
        		TBL_USER_PHOTO => $picture
        	));

        // print_r($object);exit;
        if($id == $this->cid) gf_registerCurrentUser($object);

        //$this->email->profile($this->cemail, $fname." ".$lname);

		redirect(site_url('profile/user/'.$id), 'get');
	}
